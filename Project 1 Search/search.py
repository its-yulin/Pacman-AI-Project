    ...
    while not container.isEmpty():
        ...
        if current[0] not in visited:
            ...
            for child in problem.getSuccessors(current[0]):
                container.push((child[0], current[1] + [child[1]]))
...
def uniformCostSearch(problem):
    ...
    while not container.isEmpty():
        current, actions, cost = container.pop()
        if current not in visited:
            if problem.isGoalState(current):
                return actions
            visited.append(current)
            for child in problem.getSuccessors(current):
                container.push(((child[0]), actions + [child[1]], cost + child[2]), cost + child[2])
    ...
    while not container.isEmpty():
        ...
        if current not in visited:
            ...
            if problem.isGoalState(current):
                return actions
            for child in problem.getSuccessors(current):
                container.push(((child[0]), actions + [child[1]], cost + child[2]), cost + child[2] + heuristic(child[0], problem))
