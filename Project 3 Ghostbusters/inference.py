    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            posDistribution = self.getPositionDistribution(self.setGhostPosition(gameState,oldPos))
            for newPos, prob in posDistribution.items():
                beliefs[newPos] += self.beliefs[oldPos] * prob
        beliefs.normalize()
        ...
        for p in range(self.numParticles):
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    dis = util.manhattanDistance(self.particles[p][i], pacmanPosition)
                    weight *= emissionModels[i][dis]
            possible[self.particles[p]] += weight
        if possible.totalCount() == 0:
            self.initializeParticles()
        else:
            possible.normalize()
            new = [util.sample(possible) for i in range(self.numParticles)]
            self.particles = new
